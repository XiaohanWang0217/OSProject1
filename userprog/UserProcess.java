...
public class UserProcess {
  ...
  public UserProcess() {
    /*
    // 第二题按需分配物理内存，放到load函数中生成pageTable
  ...
  public int readVirtualMemory(int vaddr, byte[] data, int offset, int length) {
    ...
    while (vpn < pageTable.length && len_read < length) {
      ...
      if (ppn<0 || ppn>=numPhysPages) break;
  ...
  protected boolean loadSections() {
    ...
    */
    ...
    if (null == pageList) return false; // 物理内存不足
    pageTable = new TranslationEntry[numPages];
    for (int i = 0; i < numPages; i++) {
      pageTable[i] =
        new TranslationEntry(
          i,
          pageList.removeFirst().intValue(),
          true,
          false,
          false,
          false
  ...
  private int handleCreat(int addr_name) {
    ...
    if (null == file_name) {
      ...
      // readVirtualMemory方法中有说明：for now, just assume that virtual addresses equal physical addresses)
      return -1;
    }
    // 最多同时打开16个文件，寻找空的文件描述符
    int i = 0;
    while (i < MAX_FILE_NUM && null != file_descriptors[i]) i++;
  ...
  // 若成功，则返回读入的字节数。如果file descriptor引用一个磁盘文件，the file position is advanced by this number.
  ...
  private int handleUnlink(int addr_name) {
    ...
    if (
      MAX_FILE_NUM == i
    ) // 不要实现任何类型的文件锁定;这是文件系统的职责。 // 没有进程打开此文件
    // 如果threadedkernel.filesysystem.open()返回一个非空的OpenFile，那么用户进程就可以访问给定的文件; 否则，您应该发出错误信号。
